            ...
            Set<String> formattedMessages = validationMessages
                ...
                .map(validationMessage -> {
                    ...
                    && !validationMessageText.contains("is not defined in the schema and the schema does not allow additional properties")) {
                        return StringUtils.substringBefore(validationMessageText, ":") + ": should match one of its valid types: " + FileReader.readFileFromClassPathOrPath(
                                "software/xdev/mockserver/schema/body.json")
                    ...
                    if (validationMessageText.startsWith("$.httpResponse.body: ")) {
                        return "$.httpResponse.body: should match one of its valid types: " + FileReader.readFileFromClassPathOrPath(
                                "software/xdev/mockserver/schema/bodyWithContentType.json")
                            .replaceAll(NEW_LINE, NEW_LINE + "   ");
                    }
